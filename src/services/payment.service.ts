...
export const createPaymentService = async (paymentData: PaymentRequestSchema) => {
  try {
    ...
    if (!paymentData.proof_of_transfer) {
      throw new HttpError('Proof of transfer is required', 400);
    }

    const createdPayment = await prisma.$transaction(async tx => {
      // Create payment from user inputs with base64 proof_of_transfer
...
export const editPaymentService = async (
  ...
  try {
    ...
    // Use new base64 proof_of_transfer if provided, otherwise keep existing
    const updatedProofOfTransfer = updatedPaymentData.proof_of_transfer || currentProofOfTransfer;
